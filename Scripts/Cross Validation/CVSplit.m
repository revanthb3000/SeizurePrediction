function[error] = CVSplit(subject)
    ...
    fourierXTrain = dlmread(strcat('Scripts/Classifiers/Fourier Model (Top Frequencies)/Parameters/',subject,'XTrain.txt'));
    fourierXTrainBiVariate = dlmread(strcat('Scripts/Classifiers/Fourier Model (Top Frequencies)/Parameters/',subject,'XTrainBiVariate.txt'));
    ...
    fourierXTrain = [varianceAndCorrelationFeatures fourierXTrain];
    ...
    for testSplitNumber = 1:K
        ...
        %SVMStruct = svmtrain(XTrain, YTrain, 'kernel_function', 'linear', 'Options', options);
%         SVMStruct = svmtrain(XTrain, YTrain, 'kernel_function', 'linear','BoxConstraint', 1);
