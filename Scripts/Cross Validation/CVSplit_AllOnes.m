function[error] = CVSplit(subject)
    ...
    varianceAndCorrelationFeaturesInterictal = dlmread(strcat('Scripts/Classifiers/Regression Tree/',subject,'interictalFeatureVectors.txt'));
    varianceAndCorrelationFeaturesPreictal = dlmread(strcat('Scripts/Classifiers/Regression Tree/',subject,'preictalFeatureVectors.txt'));
    varianceAndCorrelationFeatures=[varianceAndCorrelationFeaturesInterictal; varianceAndCorrelationFeaturesPreictal];
    %samplingXTrain = dlmread(strcat('Scripts/Classifiers/DataSampling/Parameters/',subject,'XTrain.txt'));
    %size(samplingXTrain)
    
    %Adding More Features
    %tmp=samplingXTrain;
    %samplingXTrain = [varianceAndCorrelationFeatures samplingXTrain];
    samplingXTrain=ones(size(varianceAndCorrelationFeatures));
    ...
    for testSplitNumber = 1:K
        ...
        for i=1:size(interictalDataDivison,1)
            ...
            featureVector = getSampleFeatureVector(sampleNumber,samplingXTrain);
